...
if(isset($_POST['submit']))
{
	...
	else 
	{
		...
		$sqli->table='admin';
		$field=null;
		$where=array('ad_username'=>$_POST['strUserid'], 'password'=>$pw);
		
		 $looginSQLresult=$sqli->get_selectData("SELECT * FROM admin where ad_username = '".$_POST['strUserid']."' AND password = '".$pw."' AND status = 1");
		 		if(count($looginSQLresult) >0)
		{
			$_SESSION['sessAdminID'] = $looginSQLresult[0]['adminid'];
			$_SESSION['adminUser']=$looginSQLresult[0]['ad_username'];
