...
if(isset($_REQUEST['Search']))
	{
		...
		{
			 $strcond = " AND freshleads.Reference = '".$_REQUEST['str_leadid']. "' AND  User = '". $_SESSION['sessAgentID']."'";
		}
		else
		{
			$strcond = "";
			...
			{
				$strcond.=" AND freshleads.FirstName LIKE '%".$_REQUEST['str_firstName']."%' AND  User = '". $_SESSION['sessAgentID']."'";
			}
			...
			{
					$strcond.=" AND freshleads.Status = '".$_REQUEST['dd_status']. "' AND  User = '". $_SESSION['sessAgentID']."'";
			}
			if($_REQUEST['dd_camp']!="")
			{
					$strcond.=" AND freshleads.campaignid = '".$_REQUEST['dd_camp']. "' AND  User = '". $_SESSION['sessAgentID']."'";
			}
			if($_REQUEST['dd_center']!="")
			{
					$strcond.=" AND freshleads.Cent_id = '".$_REQUEST['dd_center']. "' AND  User = '". $_SESSION['sessAgentID']."'";
			}
			if($_REQUEST['dd_stateName']!="")
			{
					$strcond.=" AND freshleads.stateName LIKE '%".$_REQUEST['dd_stateName']."%' AND  User = '". $_SESSION['sessAgentID']."'";
			}
    	 ...
    	 <?php $i=1; foreach($pager->getPage() as $key=>$row) {
      	...
      	<tr align="left">
            ...
            <a  id="data<?php echo $row['Reference'];?>" href="#" onclick="parent.L('#data<?php echo $row['Reference'];?>').colorbox({iframe:true, innerWidth:1200, innerHeight:500,href:'view_lead.php?refID=<?PHP echo $row['Reference']; ?>'});"><img src="images/search.png" alt="" title="" border="0" /></a>
              ...
              <td class="centedtd">
           <a href="update_lead.php?refID=<?PHP echo $row['Reference']; ?>"><img src="images/user_edit.png" alt="" title="" border="0" /></a>
            </td>
