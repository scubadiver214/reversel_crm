...
{
	$_SESSION['pageURL']='';
	if(isset($_SESSION['sessVerifierID']))
	{
		
		$_SESSION['sessVerifierID']='';
		unset($_SESSION['sessVerifierID']);
		//clt_comp
		$_SESSION['Verifier']='';
...
if(isset($_POST['submit'])){
	...
	else 
	{
		...
		$field=null;
		$where=array('verifier_username'=>$_POST['strUserid'], 'verifier_pwd'=>$pw);
		
		 $looginSQLresult=$sqli->get_selectData("SELECT * FROM verifier_user where verifier_username = '".$_POST['strUserid']."' AND verifier_pwd = '".$pw."' AND verifier_status = 1");	
		 		if(count($looginSQLresult) >0)
		{
			$_SESSION['sessVerifierID'] = $looginSQLresult[0]['verifier_id'];
			$_SESSION['sessVerifierUser']=$looginSQLresult[0]['verifier_username'];
