...
if(isset($_REQUEST['Search']))
	{
		...
		{
			 $strcond = " AND freshleads.Reference = '".$_REQUEST['str_leadid']. "' AND  Cent_id = '". $_SESSION['sessCenterID']."'";
		}
		else
		{
			$strcond = "";
			...
			{
				$strcond.=" AND freshleads.FirstName LIKE '%".$_REQUEST['str_firstName']."%' AND  Cent_id = '". $_SESSION['sessCenterID']."'";
			}
			if($_REQUEST['str_phone']!="")
			{
					$strcond.=" AND freshleads.Telephone1 = '".$_REQUEST['str_phone']. "' AND  Cent_id = '". $_SESSION['sessCenterID']."'";
			}
			
			if($_REQUEST['dd_client']!="")
			{
					$strcond.=" AND freshleads.Buyer = '".$_REQUEST['dd_client']. "' AND  Cent_id = '". $_SESSION['sessCenterID']."'";
			}
			if($_REQUEST['dd_status']!="")
			{
					$strcond.=" AND freshleads.Status = '".$_REQUEST['dd_status']. "' AND  Cent_id = '". $_SESSION['sessCenterID']."'";
			}
			if($_REQUEST['dd_camp']!="")
			{
					$strcond.=" AND freshleads.campaignid = '".$_REQUEST['dd_camp']. "' AND  Cent_id = '". $_SESSION['sessCenterID']."'";
			}
			
			if($_REQUEST['dd_stateName']!="")
			{
					$strcond.=" AND freshleads.stateName LIKE '%".$_REQUEST['dd_stateName']."%' AND  Cent_id = '". $_SESSION['sessCenterID']."'";
			}
    ...
    <tbody>
    	 <?php $i=1; foreach($pager->getPage() as $key=>$row) {
      	...
      	<tr>
            ...
            <td align="left"><?php echo $row['cmp_name']; ?></td>
            <td align="left"><?php echo $row['Title']." . ".$row['FirstName']." ".$row['LastName']; ?></td>
            <td align="left"><?php echo $row['Telephone1']; ?></td>
            <td align="left"><?php echo $row['clt_alias']; ?></td>
