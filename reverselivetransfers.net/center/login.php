...
if($_REQUEST['action']=='logout')
{
	...
	if(isset($_SESSION['sessCenterID']))
	{
		...
		$_SESSION['sessCenterID']='';
		unset($_SESSION['sessCenterID']);
		$_SESSION['CenterUser']='';
		unset($_SESSION['admiCenterUsernUser']);
...
if(isset($_POST['submit'])){
	...
	else 
	{
		...
		$field=null;
		$where=array('cen_username'=>$_POST['strUserid'], 'password'=>$pw);
		
		 $looginSQLresult=$sqli->get_selectData("SELECT * FROM center_user where cen_username = '".$_POST['strUserid']."' AND cen_pwd = '".$pw."' AND cen_status = 1");	
		 
		 	if(count($looginSQLresult) >0)
			{
			$_SESSION['sessCenterID'] = $looginSQLresult[0]['cenid'];
			$_SESSION['CenterUser']=$looginSQLresult[0]['cen_username'];
