...
if(isset($_REQUEST['delID']))
{
		...
		$updateInformation['ag_status']		=	0;
		$fields 										= 	array_keys($updateInformation);
		$values											=	array_values($updateInformation);
		$sqli											->	autocommit(FALSE);
		$where=array('agid'=>$_REQUEST['delID']);	
		$update_result= $sqli->Update_data($where,$values,$fields);
		if($update_result == 0)
		{
		$error	= 1;
		$sqli->rollback();
		array_push($_SESSION['errmessage'],"An Error Occurred in delete agent.");
		}	
		else
		{
		$sqli									->	commit();
		array_push($_SESSION['message'],"Agent Successfully deleted.");
...
if(isset($_POST['Search']))
{
	...
	{
		$strcond.=" AND agent_user.ag_fname LIKE '%".$_POST['str_firstName']."%' AND ag_center='".$_SESSION['sessCenterID']."'";
	}
	if($_POST['str_lastName'] !="")
	{
		$strcond.=" AND agent_user.ag_lname LIKE '%".$_POST['str_lastName']."%' AND ag_center='".$_SESSION['sessCenterID']."'";
		
	}
	if($_POST['str_email'] !="")
	{
		$strcond.=" AND agent_user.ag_email = '".$_POST['str_email']. "' AND ag_center='".$_SESSION['sessCenterID']."'";
		
		
	}
	if($_POST['str_mobile'] !="")
	{
		$strcond.=" AND agent_user.ag_mobile = '".$_POST['str_mobile']. "' AND ag_center='".$_SESSION['sessCenterID']."'";
...
$strQuery ="Select * from agent_user where ag_status=1 AND ag_center='".$_SESSION['sessCenterID']."' ".$strcond."";
$items_per_page=5;
      	...
      	<tr>
            ...
            <td class="centedtd"><?php echo $row['ag_fname']." ".$row['ag_lname']; ?></td>
             ...
             <td class="centedtd">
			   ...
			   {
				  ?> 
				<a href="?updid=<?php echo $row['agid']; ?>&stat=0&action=StatusChange&key=<?php echo md5(date("ymdhis")) ?>"> <img src="images/green.png" alt="" title="" border="0" />  </a>
			    ...
			    {
				 ?> 
				<a href="?updid=<?php echo $row['agid']; ?>&stat=1&action=StatusChange&key=<?php echo md5(date("ymdhis")) ?>"> <img src="images/red.png" alt="" title="" border="0" />  </a>
               ...
               <td style="padding-left: 10%;">
               	<a href="listagent.php?delID=<?PHP echo $row['agid']; ?>"> <img src="images/user_logout.png" alt="" title="Delete" border="0" />  </a>
